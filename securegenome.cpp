#include "data.h"
#include "securegenome.h"
#include "functions.h"
#include "roc.h"

securegenome::securegenome (string configfile) {
	gc = 0;
 ...
 */
double securegenome::singlesnplr (int genotype, double poolfreq, double reffreq) {
	double lr;
	switch (genotype){
		case 0:
 ...
 */
void securegenome::updatelrstatistics (double *totallrnull, double *totallralternate, int start, int end) {
	for (int j = 0 ; j < n; j++){
 ...
 * current set of SNPs
 */
double securegenome::getpower (double *totallrnull, double *totallralternate, int& start, int& end ) {
 ...
 */
void securegenome::search_snps (double *totallrnull, double *totallralternate, int& start, int& end, int increment, double& power, double threshold) {
	gc ++;
 ...
 */
int securegenome::find_exposed_snps_empirical ( ) {
	for (int j = 0 ; j < n; j++)
		totallrnull[j] = totallralternate[j] = 0;
 ...
 */
int securegenome::find_exposed_snps_analytical () {
...
int main (int argc, char *argv[]){ 
	if (argc > 1) { 
		securegenome *sg = new securegenome (argv[1]);
